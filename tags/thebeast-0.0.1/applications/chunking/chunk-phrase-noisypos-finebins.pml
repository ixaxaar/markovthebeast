...
factor:
  ...
  add [chunk(b,e,c)] * w_noisycpos_i(w_i, i - b,bins(0,1,2,3,4,5,10,e-b), c);
...
factor:
  ...
  add [chunk(b,e,c)] * w_bi_noisycpos(w1,w2,bins(0,1,2,3,4,5,10,e-b),c);
...
factor:
  ...
  add [chunk(b,e,c)] * w_tri_noisycpos(w1,w2,w3, bins(0,1,2,3,4,5,10,e-b),c);
...
factor:
  ...
  add [chunk(b,e,c)] * w_before_noisycpos(w,bins(0,1,2,3,4,5,10,e-b),c);

weight w_after_noisycpos: Noisycpos x Int x Chunk -> Double;
factor:
  for Int b, Int e, Chunk c, Noisycpos w
  if noisycpos(b,_) & noisycpos(e,_) & e >= b & noisycpos(e+1,w)
  add [chunk(b,e,c)] * w_after_noisycpos(w,bins(0,1,2,3,4,5,10,e-b),c);
