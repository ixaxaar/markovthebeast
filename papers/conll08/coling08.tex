...
\title{Instructions for Coling 2008 Proceedings}
...
\begin{abstract}
  ...
  manuscript for the proceedings of Coling 2008. The document itself
...
This document has been adapted from the instructions for ACL-07, adapted from
those for ACL/Coling-06, which were in turn adapted from those for EACL-06,
which were in turn adapted from the instructions for ACL-05 and EACL-03.  All
these were based on the formats of earlier ACL and EACL Conference
proceedings. Those versions were written by several people, including John
Chen, Henry S. Thompson and Donald Walker.

\section{Introduction}

The following formatting instructions are directed to authors of
papers accepted for publication in Coling 2008 proceedings, including
the main conference, workshops, posters and demos. The
conference website http://www.coling2008.org.uk has additional submission
...
chair Roger Evans (R.P.Evans@brighton.ac.uk) as soon as possible.
...
Unless otherwise specified, the maximum length of a manuscript is $8$ pages
for regular and workshop papers and $4$ pages for posters and demos, printed
single-sided (see Section~\ref{sec:length} for additional information on the
maximum number of pages).

\subsection{Electronically-available resources}

This description is provided in \LaTeX2e (\nobreak{coling08.tex}) along
with the \LaTeX2e style file used to format it (\nobreak{coling08.sty})
and a Coling bibliography style (\nobreak{coling.bst}); and in PDF format
(\nobreak{coling08.pdf}). These files are all available at
http://www.coling08.org.uk/style.html. There is also a Microsoft Word
document template (\nobreak{coling08.dot}) available at the same URL. We
strongly recommend the use of these style files, which have been
appropriately tailored for the Coling 2008 proceedings.
...
production of your camera-ready paper, please contact the publications chair
as soon as possible.
...
as linguistic phrases; for example, instead of ``\cite{Gusfield:97} argues
that \ldots'' use ``Gusfield~\shortcite{Gusfield:97} argues that \ldots''.
Append lowercase letters to the year in cases of ambiguity. Treat double
authors as in~\cite{Aho:72}, but write as in~\cite{Chandra:81} when more than
two authors are involved. Collapse multiple citations in parenthesis as
...
Unless otherwise specified, the maximum length is $8$ pages for regular and
workshop papers and $4$ pages for posters and demos. The page limit should be
