...
\begin{table}
    ...
    \begin{tabular}{|c|p{5cm}|}
        \hline
    \emph{word}    & Orthography of the token.\\\hline
    \emph{lemma}   & Predicted lemma for the form of the token.\\\hline
    \emph{ppos}    & Predicted POS tag for the form of the token.\\\hline
    \emph{cpos}    & Predicted coarse POS tag for the form of the token.\\\hline
    \emph{possiblePre} & True if the token can be a predicate based on inspecting its POS tag.\\\hline
    \emph{possibleArg} & True if the form can be a argument based on inspecting its POS tag.\\\hline
    \emph{voice}   & Voice of the form if it is a verb (Active/Passive).\\\hline
    \emph{dep}     & Syntactic dependency relation.\\\hline
    \emph{head}    & Syntactic head of the current token.\\\hline
    \emph{palmer}  & True if prunned by heuristics proposed in
    \citep{xue04calibrating}.\\\hline
    \emph{depPath}      & Syntactic dependency path among two tokens (we include
    the unlabelled and length version).\\\hline
    \emph{depFrame}     & Syntactic dependency frame for a token as proposed by \citet{xue04calibrating}(we include the
    unlabelled version).\\\hline
    \emph{depPathFrame} & Syntactic dependency frame path among two tokens  as proposed by \citet{xue04calibrating} (we include the unlabelled and distance version).\\\hline
    \end{tabular}
    \caption{Observable predicates}
    \label{tbl:o_preds}
\end{table}
...
Instead of listing whole set of local formulae in more detail we refer the reader
