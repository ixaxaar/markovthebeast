...
factor: for Int p, Int a if lemma(p,_) & lemma(a,_) add [hasLabel(p,a)] * l_hasLabel_dist(bins(0,1,2,3,4,5,10,a-p));
...
/*
weight w_hasLabel_impossibleArgument: Double;
factor: for Int p, Int a if lemma(p,_) & lemma(a,_) & !possibleArgument(a)
  add [hasLabel(p,a)] * w_hasLabel_impossibleArgument;

weight w_hasLabel_impossiblePredicateAndArgument: Double;
factor: for Int p, Int a if lemma(p,_) & lemma(a,_) & !possiblePredicate(p) & !possibleArgument(a)
  add [hasLabel(p,a)] * w_hasLabel_impossiblePredicateAndArgument;

weight w_hasLabel_notPalmer: Double;
factor: for Int p, Int a if lemma(p,_) & lemma(a,_) & !palmer(p,a)
  add [hasLabel(p,a)] * w_hasLabel_notPalmer;
...
factor: for Int p, Int a, Lemma l_p, Lemma l_a if lemma(p,l_p) & lemma(a,l_a)
...
factor: for Int p, Int a, Ppos p_p, Ppos p_a if ppos(p,p_p) & ppos(a,p_a)
...
factor: for Int p, Int a, Ppos p_p, Ppos p_a if ppos(p,p_p) & ppos(a+1,p_a)
...
factor: for Int p, Int a, Lemma l_p, Ppos p_a if lemma(p,l_p) & ppos(a,p_a)
...
factor: for Int p, Int a, Ppos p_p, Lemma l_a if ppos(p,p_p) & lemma(a,l_a)
  add [hasLabel(p,a)] * l_hasLabel_pp_la(p_p,l_a);

//predicate pos and argument wnet
weight l_hasLabel_pp_wna: Ppos x WNet -> Double;
factor: for Int p, Int a, Ppos p_p, WNet l_a if ppos(p,p_p) & wnet(a,l_a)
  add [hasLabel(p,a)] * l_hasLabel_pp_wna(p_p,l_a);

//predicate pos and argument wnet + dist
weight l_hasLabel_pp_wna_dist: Ppos x WNet x Int -> Double;
factor: for Int p, Int a, Ppos p_p, WNet l_a if ppos(p,p_p) & wnet(a,l_a)
  add [hasLabel(p,a)] * l_hasLabel_pp_wna_dist(p_p,l_a,bins(0,1,2,3,4,5,10,a-p));

//same token features: lemma
weight w_same_lemma: Lemma -> Double;
factor: for Int p, Lemma l if lemma(p,l) add [hasLabel(p,p)] * w_same_lemma(l);

//same token features: ppos
weight w_same_ppos: Ppos -> Double;
factor: for Int p, Ppos l if ppos(p,l) add [hasLabel(p,p)] * w_same_ppos(l);

//same token features: wnet
weight w_same_wnet: WNet -> Double;
factor: for Int p, WNet l if wnet(p,l) add [hasLabel(p,p)] * w_same_wnet(l);
