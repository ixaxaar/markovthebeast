...
factor: for Int a, Int p, MPathFrame pathFrame if m_path_frame(p,a,pathFrame)
...
factor: for Int a, Int p, MPathFrame pathFrame, Voice v if m_path_frame(p,a,pathFrame) & voice(p,v)
...
factor: for Int a, Int p, MPathFrame pathFrame, Lemma l if m_path_frame(p,a,pathFrame) & lemma(p,l)
...
factor: for Int a, Int p, MPathFrame pathFrame, WNet l if m_path_frame(p,a,pathFrame) & wnet(p,l)
...
factor: for Int a, Int p, MPathFrame pathFrame, WNet l if m_path_frame(p,a,pathFrame) & wnet(a,l)
...
factor: for Int a, Int p, MPathFrameUnlabeled pathFrame if m_path_frame_unlabeled(p,a,pathFrame)
  add[hasLabel(p,a)] * w_dep_path_frame_unlabeled(pathFrame);

//dep path between two tokens
weight l_hasLabel_path:  MPath -> Double;
factor: for Int a, Int p,  MPath path if lemma(a,_) & lemma(p,_) & m_path(a,p,path)
  add [hasLabel(p,a)] * l_hasLabel_path(path);

//dep path between two tokens & voice
weight l_hasLabel_path_voice:  MPath x Voice -> Double;
factor: for Int a, Int p,  MPath path, Voice v if lemma(a,_) & lemma(p,_) & m_path(a,p,path) & voice(p,v)
...
factor: for Int a, Int p,  MPathUnlabeled path if lemma(a,_) & lemma(p,_) & m_path_unlabeled(a,p,path)
...
factor: for Int a, Int p,  MPathUnlabeled path, Voice v if lemma(a,_) & lemma(p,_) & m_path_unlabeled(a,p,path) & voice(p,v)
...
factor: for Int a, Int p,  MPath path, Lemma l_hasLabel_p if lemma(a,_) & lemma(p,l_hasLabel_p) & m_path(a,p,path)
...
factor: for Int a, Int p,  MPath path, WNet l_hasLabel_p if wnet(a,_) & wnet(p,l_hasLabel_p) & m_path(a,p,path)
...
factor: for Int a, Int p,  MPathUnlabeled path, Lemma l_hasLabel_p if lemma(a,_) & lemma(p,l_hasLabel_p) & m_path_unlabeled(a,p,path)
...
factor: for Int a, Int p,  MPathUnlabeled path, WNet l_hasLabel_p if wnet(a,_) & wnet(p,l_hasLabel_p) & m_path_unlabeled(a,p,path)
...
factor: for Int a, Int p,  MPath path, Lemma l_hasLabel_a if lemma(a,l_hasLabel_a) & lemma(p,_) & m_path(a,p,path)
...
factor: for Int a, Int p,  MPath path, WNet l_hasLabel_a if wnet(a,l_hasLabel_a) & wnet(p,_) & m_path(a,p,path)
  add [hasLabel(p,a)] * l_hasLabel_path_wnet_a(path,l_hasLabel_a);

//dep frame for predicate
weight l_hasLabel_frame_p: MFrame -> Double;
factor: for Int a, Int p,  MFrame frame if lemma(a,_) & lemma(p,_) & m_frame(p,frame)
  add [hasLabel(p,a)] * l_hasLabel_frame_p(frame);

//unlabeled dep frame
weight l_hasLabel_frame_unlabeled_p:  MFrameUnlabeled -> Double;
factor: for Int a, Int p,  MFrameUnlabeled frame if lemma(a,_) & lemma(p,_) & m_frame_unlabeled(p,frame)
...
factor: for Int a, Int p,  MFrame frame, Lemma l_hasLabel_p if lemma(a,_) & lemma(p,l_hasLabel_p) & m_frame(p,frame)
...
factor: for Int a, Int p,  MFrame frame, WNet l_hasLabel_p if wnet(a,_) & wnet(p,l_hasLabel_p) & m_frame(p,frame)
...
factor: for Int a, Int p,  MFrameUnlabeled frame, Lemma l_hasLabel_p if lemma(a,_) & lemma(p,l_hasLabel_p) & m_frame_unlabeled(p,frame)
...
factor: for Int a, Int p,  MFrame frame, Lemma l_hasLabel_a if lemma(a,l_hasLabel_a) & lemma(p,_) & m_frame(p,frame)
...
factor: for Int a, Int p,  MFrame frame, WNet l_hasLabel_a if wnet(a,l_hasLabel_a) & wnet(p,_) & m_frame(p,frame)
  add [hasLabel(p,a)] * l_hasLabel_frame_p_wnet_a(frame,l_hasLabel_a);

//same token features: m_frame
weight w_same_m_frame: MFrame -> Double;
factor: for Int p, MFrame l if m_frame(p,l) add [hasLabel(p,p)] * w_same_m_frame(l);

//same token features: m_frame_unlabeled
weight w_same_m_frame_unlabeled: MFrameUnlabeled -> Double;
factor: for Int p, MFrameUnlabeled l if m_frame_unlabeled(p,l) add [hasLabel(p,p)] * w_same_m_frame_unlabeled(l);


//pp attachment features
weight w_pp_attach1: Ppos x Ppos -> Double;
factor: for Int p, Int a, Int m, Ppos pm, Ppos pp if ppos(p,pp) & ppos(a,"IN") & m_dep(a,m,_) & ppos(m,pm)
  add [hasLabel(p,a)] * w_pp_attach1(pp,pm);

weight w_pp_attach2: Ppos x WNet -> Double;
factor: for Int p, Int a, Int m, WNet pm, Ppos pp if ppos(p,pp) & ppos(a,"IN") & m_dep(a,m,_) & wnet(m,pm)
  add [hasLabel(p,a)] * w_pp_attach2(pp,pm);

weight w_pp_attach3: WNet x Ppos -> Double;
factor: for Int p, Int a, Int m, WNet pp, Ppos pm if wnet(p,pp) & ppos(a,"IN") & m_dep(a,m,_) & ppos(m,pm)
  add [hasLabel(p,a)] * w_pp_attach3(pp,pm);

weight w_pp_attach4: Ppos x Lemma -> Double;
factor: for Int p, Int a, Int m, Lemma pm, Ppos pp if ppos(p,pp) & ppos(a,"IN") & m_dep(a,m,_) & lemma(m,pm)
  add [hasLabel(p,a)] * w_pp_attach4(pp,pm);

weight w_pp_attach5: Lemma x Ppos -> Double;
factor: for Int p, Int a, Int m, Ppos pm, Lemma pp if lemma(p,pp) & ppos(a,"IN") & m_dep(a,m,_) & ppos(m,pm)
  add [hasLabel(p,a)] * w_pp_attach5(pp,pm);

/*
//generic attachment features
weight w_hasLabel_attach: MDependency x MDependency -> Double;
factor: for Int p, Int a, Int m, MDependency dpa, MDependency dam if m_dep(p,a,dpa) & m_dep(a,m,dam)
  add [hasLabel(p,a)] * w_hasLabel_attach(dpa,dam);

weight w_hasLabel_attach_lp: MDependency x MDependency x Lemma -> Double;
factor: for Int p, Int a, Int m, MDependency dpa, MDependency dam, Lemma lp if m_dep(p,a,dpa) & m_dep(a,m,dam) & lemma(p,lp)
  add [hasLabel(p,a)] * w_hasLabel_attach_lp(dpa,dam,lp);

weight w_hasLabel_attach_la: MDependency x MDependency x Lemma -> Double;
factor: for Int p, Int a, Int m, MDependency dpa, MDependency dam, Lemma la if m_dep(p,a,dpa) & m_dep(a,m,dam) & lemma(a,la)
  add [hasLabel(p,a)] * w_hasLabel_attach_la(dpa,dam,la);

weight w_hasLabel_attach_pa: MDependency x MDependency x Ppos -> Double;
factor: for Int p, Int a, Int m, MDependency dpa, MDependency dam, Ppos pa if m_dep(p,a,dpa) & m_dep(a,m,dam) & ppos(a,pa)
  add [hasLabel(p,a)] * w_hasLabel_attach_pa(dpa,dam,pa);

weight w_hasLabel_attach_wa: MDependency x MDependency x WNet -> Double;
factor: for Int p, Int a, Int m, MDependency dpa, MDependency dam, WNet wa if m_dep(p,a,dpa) & m_dep(a,m,dam) & wnet(a,wa) 
...
factor: for Int a, Int p,  MFrame frame if lemma(a,_) & lemma(p,_) & m_frame(a,frame)
