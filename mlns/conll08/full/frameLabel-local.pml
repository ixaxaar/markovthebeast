...
factor: for Int p, FrameLabel l if word(p,_) add [frameLabel(p,l)] * w_frameLabel(l);

weight w_frameLabel_slemma: Slemma x FrameLabel -> Double;
factor: for Int p, FrameLabel l, Slemma s if slemma(p,s) add [frameLabel(p,l)] * w_frameLabel_slemma(s,l);
...
//factor: for Int i, FrameLabel f, MDependency m if word(i,_) & m_dep(_,i,m) add [frameLabel(i,f)] * w_fl_mdep(m,f);

//weight w_fl_mframe: MFrame x FrameLabel -> Double;
//factor: for Int i, FrameLabel fl, MFrame f if word(i,_) & m_frame(i,f) add [frameLabel(i,fl)] * w_fl_mframe(f,fl);
