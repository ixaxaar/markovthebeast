...
factor[0]: for Int p if word(p,_) & possiblePredicate(p): |Int a: possibleArgument(a) & hasLabel(p,a)| >= 1;

// if a tokenpair is labelled the first token must be a predicate
//factor: for Int p, Int a if word(a,_) & word(p,_) & possiblePredicate(p) : | hasLabel(p,a) | >= 1;
