...
factor[3]: for Int a, Int p, Lemma l, FrameLabel s, Lemma la if
  lemma(p,l) & lemma(a,la) &  possiblePredicate(p) & possibleArgument(a) add [ frameLabel(p,s) & isArgument(a)] * w_iag_predLemmaSense(l,la,s);
...
factor[2]: for Int a, Int p, Lemma l, Lemma la if
  lemma(p,l) & lemma(a,la) & possiblePredicate(p) & possibleArgument(a) add [ isArgument(a)] * w_iag_predLemma(l,la);
