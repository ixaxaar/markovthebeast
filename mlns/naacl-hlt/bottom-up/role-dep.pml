...
factor: for Int a, Int p, Role r, MDependency d_a, MDependency d_p if mst_dep(_,a,d_a) & mst_dep(_,p,d_p)
  add[role(p,a,r)] * w_role_dep_p_a(d_p,d_a,r);

//dep label of predicate
weight w_role_dep_p: MDependency x Role -> Double;
factor: for Int a, Int p, Role r, MDependency d_p if mst_dep(_,a,_) & mst_dep(_,p,d_p)
  add[role(p,a,r)] * w_role_dep_p(d_p,r);

//dep label of argument
weight w_role_dep_a: MDependency x Role -> Double;
factor: for Int a, Int p, Role r, MDependency d_a if mst_dep(_,a,d_a) & mst_dep(_,p,_)
  add[role(p,a,r)] * w_role_dep_a(d_a,r);

//dep path length between two tokens
weight w_role_path_length: Int x Role -> Double;
factor: for Int a, Int p, Role r, Int length if word(a,_) & word(p,_) & mst_path_length(p,a,length)
...
factor: for Int a, Int p, Role r, MPathFrame pathFrame, Lemma l, Voice v if mst_path_frame(p,a,pathFrame) & lemma(p,l) & voice(p,v)
