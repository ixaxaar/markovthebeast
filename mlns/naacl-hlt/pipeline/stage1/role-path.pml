...
factor: for Int a, Int p, Role r, MPath path, Ppos pa if mst_path(p,a,path) & ppos(a,pa) &
...
factor: for Int a, Int p, Role r, MPath path, Cpos pa, Slemma lp if mst_path(p,a,path) & cpos(a,pa) & slemma(p,lp) & 
  possiblePredicate(p) & possibleArgument(a) &
  palmer(p,a)
...
factor: for Int a, Int p, Role r, MPath path, Slemma la if mst_path(p,a,path) & slemma(a,la) &
  possiblePredicate(p) & possibleArgument(a) &
  palmer(p,a)
...
factor: for Int a, Int p, Role r, MPath path, Slemma la, Slemma lp if mst_path(p,a,path) & slemma(a,la) & slemma(p,lp) &
  possiblePredicate(p) & possibleArgument(a) &
  palmer(p,a)
...
//  mst_path(p,a,path) & slemma(a,la) & slemma(p,lp) & wnet(a,wa) &
//  possiblePredicate(p) & possibleArgument(a) &
//  palmer(p,a)
