...
factor: for Int a, Int p, Role r, MDependency d_a, MDependency d_p if mst_dep(_,a,d_a) & mst_dep(_,p,d_p) & possiblePredicate(p) & possibleArgument(a)
  add[role(p,a,r)] * w_role_dep_p_a(d_p,d_a,r);

//dep label of predicate
weight w_role_dep_p: MDependency x Role -> Double;
factor: for Int a, Int p, Role r, MDependency d_p if mst_dep(_,a,_) & mst_dep(_,p,d_p) & possiblePredicate(p) & possibleArgument(a)
  add[role(p,a,r)] * w_role_dep_p(d_p,r);

//dep label of argument
weight w_role_dep_a: MDependency x Role -> Double;
factor: for Int a, Int p, Role r, MDependency d_a if mst_dep(_,a,d_a) & mst_dep(_,p,_) & possiblePredicate(p) & possibleArgument(a)

  add[role(p,a,r)] * w_role_dep_a(d_a,r);

//dep path length between two tokens
weight w_role_path_length: Int x Role -> Double;
factor: for Int a, Int p, Role r, Int length if word(a,_) & word(p,_) & mst_path_length(p,a,length) & possiblePredicate(p) & possibleArgument(a)
...
factor: for Int a, Int p, Role r, MPathFrame pathFrame if mst_path_frame(p,a,pathFrame)  & possiblePredicate(p) & possibleArgument(a)
...
factor: for Int a, Int p, Role r, MPathFrame pathFrame, Voice v if mst_path_frame(p,a,pathFrame) & voice(p,v) & possiblePredicate(p) & possibleArgument(a)
...
factor: for Int a, Int p, Role r, MPathFrame pathFrame, Lemma l if mst_path_frame(p,a,pathFrame) & lemma(p,l)  & possiblePredicate(p) & possibleArgument(a)
...
factor: for Int a, Int p, Role r, MPathFrame pathFrame, Lemma l, Voice v if mst_path_frame(p,a,pathFrame) & lemma(p,l) & voice(p,v) & lemma(p,l)  & possiblePredicate(p) & possibleArgument(a)
...
factor: for Int a, Int p, Role r, MPathFrameUnlabeled pathFrame if mst_path_frame_unlabeled(p,a,pathFrame)  & possiblePredicate(p) & possibleArgument(a)
...
//factor: for Int a, Int p, Role r, MPath path if lemma(a,_) & lemma(p,_) & mst_path(a,p,path)
//  add [role(p,a,r)] * l_path(r,path);

//dep path between two tokens and voice
//weight l_path_voice: Role x MPath x Voice -> Double;
//factor: for Int a, Int p, Role r, MPath path, Voice v if lemma(a,_) & lemma(p,_) & mst_path(a,p,path) & voice(p,v)
//  add [role(p,a,r)] * l_path_voice(r,path,v);

//unlabeled dep path
//weight l_path_unlabeled: Role x MPathUnlabeled -> Double;
//factor: for Int a, Int p, Role r, MPathUnlabeled path if lemma(a,_) & lemma(p,_) & mst_path_unlabeled(a,p,path)
//  add [role(p,a,r)] * l_path_unlabeled(r,path);

//dep path and lemma of predicate
//weight l_path_lemma_p: Role x MPath x Lemma -> Double;
//factor: for Int a, Int p, Role r, MPath path, Lemma l_p if lemma(a,_) & lemma(p,l_p) & mst_path(a,p,path)
...
//factor: for Int a, Int p, Role r, MPathUnlabeled path, Lemma l_p if lemma(a,_) & lemma(p,l_p) & mst_path_unlabeled(a,p,path)
...
//factor: for Int a, Int p, Role r, MPath path, Lemma l_a if lemma(a,l_a) & lemma(p,_) & mst_path(a,p,path)
//  add [role(p,a,r)] * l_path_lemma_a(r,path,l_a);

//dep frame for predicate
//weight l_frame_p: Role x MFrame -> Double;
//factor: for Int a, Int p, Role r, MFrame frame if lemma(a,_) & lemma(p,_) & mst_frame(p,frame)
//  add [role(p,a,r)] * l_frame_p(r,frame);

//dep frame and lemma of predicate
//weight l_frame_lemma_p: Role x MFrame x Lemma -> Double;
//factor: for Int a, Int p, Role r, MFrame frame, Lemma l_p if lemma(a,_) & lemma(p,l_p) & mst_frame(p,frame)
...
//factor: for Int a, Int p, Role r, MFrame frame, Lemma l_a if lemma(a,l_a) & lemma(p,_) & mst_frame(p,frame)
...
factor: for Int a, Int p, Role r, MFrame frame if lemma(a,_) & lemma(p,_) & mst_frame(a,frame)
